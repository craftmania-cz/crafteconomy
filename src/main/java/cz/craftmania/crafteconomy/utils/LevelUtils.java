...
public interface LevelUtils {
    ...
    static double getExactLevel(double exp) {
        return LevelUtils.getLevel(exp) + LevelUtils.getPercentageToNextLevel(exp);
    }
    ...
    static double getTotalExpToLevel(double level) {
        double lv = Math.floor(level), x0 = LevelUtils.getTotalExpToFullLevel(lv);
        if (level == lv) return x0;
        return (LevelUtils.getTotalExpToFullLevel(lv + 1) - x0) * (level % 1) + x0;
    }
